...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        if TRAIN_DS == 'C+T+K+S+H':
            ...
            print(type(train_dataset[2][2]),train_dataset[2][2].shape)
            #<class 'torch.Tensor'> torch.Size([3, 384, 512])
        elif TRAIN_DS == 'C+T+K/S':
            train_dataset = 100*sintel_clean + 100*sintel_final + 10*things
        ...
        cai_train_path = '/media/newdisk/datasets_piv/piv_cai/train'
        cai_train_data = DatasetPIV(cai_train_path, transform=to_tensor)
        train_dataset = cai_train_data

    elif args.stage == 'P2':
        to_tensor = transforms.Compose([
            transforms.ToTensor()  # Converts a PIL Image or numpy.ndarray to tensor.
        ])
        raft256_train_path = '/media/newdisk/datasets_piv/piv_raft/Data_ProblemClass2_RAFT-PIV/Training_Dataset_ProblemClass2_RAFT256-PIV'
        raft_train_data = DatasetPIV(raft256_train_path, transform=to_tensor)
        train_dataset = raft_train_data
