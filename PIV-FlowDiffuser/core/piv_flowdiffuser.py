...
class FlowDiffuser(nn.Module):
    ...
    def forward(self, image1, image2, test_mode=False, iters=None, flow_gt=None, flow_init=None):
        ...
        scale_factor = 2
        image1 = F.interpolate(image1, scale_factor=scale_factor, mode='bilinear', align_corners=True)
        image2 = F.interpolate(image2, scale_factor=scale_factor, mode='bilinear', align_corners=True)
        #######################scale up###############################

        if flow_gt is not None:
            flow_gt = F.interpolate(flow_gt, scale_factor=scale_factor, mode='bilinear', align_corners=True) * scale_factor
        if flow_init is not None:
            flow_init = F.interpolate(flow_init, scale_factor=scale_factor, mode='bilinear', align_corners=True) * scale_factor
        ...
        #######################scale down###############################
        flow_list = [F.interpolate(flow, size=original_size, mode='bilinear', align_corners=True) / scale_factor for flow in flow_list]
        #######################scale down###############################

        
        if test_mode:
            flow = coords1 - coords0
            flow = F.interpolate(flow, size=original_size, mode='bilinear', align_corners=True) / scale_factor
