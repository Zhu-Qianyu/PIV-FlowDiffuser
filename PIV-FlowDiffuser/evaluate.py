...
def validate_CAI(model, iters=12):
    model.eval()
    ...
    aae_list, rmse_list, aee_list = [], [], []

    print(len(val_dataset))


    for val_id in range(50):
        image1, image2, flow_gt, valid_gt = val_dataset[val_id*4]
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()
        ...
        aae_list.append(metrics.AAE(ut, vt, um, vm))
        rmse_list.append(metrics.RMSE(ut, vt, um, vm))
        aee_list.append(metrics.AEE(ut, vt, um, vm))

    all_aae_data = np.concatenate([aae_data.flatten() for aae_data in aae_list])
    all_rmse_data = np.concatenate([rmse_data.flatten() for rmse_data in rmse_list])
    all_aepe_data = np.concatenate([aepe_data.flatten() for aepe_data in aee_list])

    aae = np.average(all_aae_data)
    rmse = np.average(all_rmse_data)
    aepe = np.average(all_aepe_data)

    print("Validation CAI: AAE=%f, RMSE=%f, AEE=%f" % (aae, rmse, aepe))
    return {'CAI-epe': aae, 'CAI-rmse': rmse, 'CAI-aee': aepe}
